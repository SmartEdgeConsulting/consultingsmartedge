...
import type { SanityJob } from "@/src/sanity/types";
...
async function syncAllJobs() {
  ...
  try {
    ...
    const sanityJobs: SanityJob[] = await serverClient.fetch(getJobs);
    ...
    console.log(
      `✅ Successfully synced ${successCount}/${sanityJobs.length} jobs`
    );
